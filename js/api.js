...
export async function getOrders({ keyword = "", status = "", dateFrom = "", dateTo = "", limit = 30, offset = 0 } = {}) {
  let q = sb
    .from("orders")
    .select("*, customers(name), order_items(id), created_by_profile:profiles!orders_created_by_fkey(display_name), paid_by_profile:profiles!orders_paid_by_fkey(display_name), voided_by_profile:profiles!orders_voided_by_fkey(display_name)", { count: "exact" })
